...
import { countNfts, fetchNfts, reservoirCountNfts, reservoirFetchNfts } from './services/nfts/nft.service';
 ...
 * the endpoint of the deployed Cloud Function
 */
 export async function reservoirNfts(req: Request, res: Response) {
  res.set('Access-Control-Allow-Origin', '*');
  res.set('Content-Type', 'application/json');
  ...
  try {
    ...
    switch (req.query.action) {
      case CloudActions.QUERY:
        result = await reservoirFetchNfts(req.query);
        break;
      case CloudActions.COUNT:
        result = await reservoirCountNfts(req.query);
        break;
    }
...
mimicking the behavior of the cloud function endpoint
*/
if (config.node_env !== 'production') {
  ...
  app.get('/reservoir-nfts', (req, res) => {
    reservoirNfts(req, res);
  });
