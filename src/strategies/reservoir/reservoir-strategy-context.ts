...
import {
  ...
  ReservoirOwnerStrategy,
  ReservoirNftOwnerOrderStrategy,
  ReservoirNftOrderStrategy,
} from '../../strategies';
import { CloudActions } from '../../validations';
import { utils } from 'ethers';

// The context defines the interface of interest to clients.
export class ReservoirStrategyContext {
  ...
  public constructor(params: IExecutionParameters) {
    ...
    if (onlyNftsParams) {
      this.setStrategy(new ReservoirNftStrategy());
    }

    if (onlyOwnerParams) {
      this.setStrategy(new ReservoirOwnerStrategy());
    }
    ...
    if (
      ...
      this.setStrategy(new ReservoirNftOwnerStrategy());
    }
    ...
    if (
      ...
      this.setStrategy(new ReservoirNftOrderStrategy());
    }

    if (hasNftParams && hasOrderParams && hasOwnerParams && !hasHistoryParams) {
      this.setStrategy(new ReservoirNftOwnerOrderStrategy());
    }
